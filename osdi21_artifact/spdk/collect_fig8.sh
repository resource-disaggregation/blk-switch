...
for qd in 1 2 4 8 16 32; do
    paste <(echo $qd) <(cat $prefix-qd$qd.lat*.txt | grep "Total        " | awk 'BEGIN{x=0;} {x += $5;} END{print x/NR;}') <(cat $prefix-qd$qd.lat*.txt | grep "99.00000" | tr -d 'us' | awk 'BEGIN{x=0;} {x += $3;} END{print x/NR;}');
done
echo ""
...
for qd in 1 2 4 8 16 32; do
    ...
    paste <(echo $qd) <(awk -v txp=$tapp_xput -v lxp=$lapp_xput -v n=$num_cores 'BEGIN {print (txp+lxp), (txp+lxp)/n}');
done