...
echo "(0: isolated case)";
for num_lat in 0 1 2 4; do
    paste <(echo $num_lat) <(cat $prefix-lat$num_lat.lat*.txt | grep "Total        " | awk 'BEGIN{x=0;} {x += $5;} END{print x/NR;}') <(cat $prefix-lat$num_lat.lat*.txt | grep "99.00000" | tr -d 'us' | awk 'BEGIN{x=0;} {x += $3;} END{print x/NR;}');
done
echo ""
...
for num_lat in 0 1 2 4; do
    ...
    paste <(echo $num_lat) <(awk -v txp=$tapp_xput -v lxp=$lapp_xput -v n=$num_cores 'BEGIN {print (txp+lxp), (txp+lxp)/n}');
done
