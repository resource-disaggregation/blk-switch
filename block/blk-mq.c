...
retry:
	...
	if (blk_switch_request(data->bio, data)) {
		...
		if (blk_switch_debug && blk_switch_reset_metrics &&
		   blk_switch_stats_print[current->thread_info.cpu]) {
			printk("(pid %d cpu %2d) reqstr stats: gen %8d str %8d prc %8d",
				current->pid, current->thread_info.cpu,
				blk_switch_stats_gen[current->thread_info.cpu],
				blk_switch_stats_str[current->thread_info.cpu],
				blk_switch_stats_prc[current->thread_info.cpu]);

			blk_switch_stats_gen[current->thread_info.cpu] = 0;
			blk_switch_stats_str[current->thread_info.cpu] = 0;
			blk_switch_stats_prc[current->thread_info.cpu] = 0;
			blk_switch_stats_print[current->thread_info.cpu] = 0;
		}
	}
	...
	if (blk_switch_on >= 2 && blk_switch_request(data->bio, data)) {
		...
		if (current->thread_info.cpu != cur_cpu) {
			data->ctx = per_cpu_ptr(q->queue_ctx, current->thread_info.cpu);
			data->hctx = blk_mq_map_queue(q, data->cmd_flags, data->ctx);
			app_steered = true;
		}
	}
